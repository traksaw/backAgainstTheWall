...
const singleQuestionLayout: QuizQuestion[] = [
  {
    id: 1,
    text: 'Q1',
    options: [
      { id: 1, text: 'Architect pick', archetype: 'Architect', points: 3 },
      { id: 2, text: 'Avoider pick', archetype: 'Avoider', points: 5 },
      { id: 3, text: 'Gambler pick', archetype: 'Gambler', points: 4 },
      { id: 4, text: 'Realist pick', archetype: 'Realist', points: 2 },
    ],
  },
]

function mockInProgressAttempt(layout: QuizQuestion[] = singleQuestionLayout) {
  const attempt = {
    layout,
    status: 'in_progress',
  ...
  findOneMock.mockResolvedValue(attempt)
  return attempt
}
...
describe('POST /api/quiz/submit (WAS-8: reject malformed bodies before they reach Mongoose)', () => {
  ...
  it('rejects a payload missing sessionId', async () => {
    const res = await POST(makeRequest({ answers: validPayload.answers }))
...
describe('POST /api/quiz/submit (WAS-89: recompute archetype/score server-side, never trust the client)', () => {
  ...
  it('rejects more answers than the persisted layout has questions instead of letting a flood inflate a score', async () => {
    mockInProgressAttempt(singleQuestionLayout)
    const answers: Record<number, { archetype: string; points: number }> = {}
    for (let i = 0; i <= 5; i++) {
      answers[i] = { archetype: 'Architect', points: 3 }
    }
...
describe('POST /api/quiz/submit (WAS-107: verify answers against persisted shuffle layout)', () => {
  beforeEach(() => {
    createMock.mockReset()
    findOneMock.mockReset()
    getUserIdFromRequestMock.mockReset()
  ...
  it('rejects a crafted answer that does not match any option shown at that position', async () => {
    mockInProgressAttempt(singleQuestionLayout)

    const res = await POST(
      makeRequest({
        // Avoider with 4 was never shown — layout has Avoider at 5
        answers: { 0: { archetype: 'Avoider', points: 4 } },
  ...
  it('rejects submit when there is no in-progress attempt for the sessionId', async () => {
  ...
  it('scores a normal matching submission correctly and marks the attempt completed', async () => {
    const attempt = mockInProgressAttempt(singleQuestionLayout)
    createMock.mockResolvedValue({ _id: 'result-ok' })

    const res = await POST(
      makeRequest({
        answers: { 0: { archetype: 'Gambler', points: 4 } },
    ...
    expect(createMock).toHaveBeenCalledTimes(1)
    const [data] = createMock.mock.calls[0]
    expect(data.archetype).toBe('Gambler')
